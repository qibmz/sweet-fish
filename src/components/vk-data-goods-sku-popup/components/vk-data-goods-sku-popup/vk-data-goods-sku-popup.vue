...
<script>
import vkDataInputNumberBox from '@/components/vk-data-goods-sku-popup/components/vk-data-input-number-box/vk-data-input-number-box.vue'

let vk // vk依赖
const goodsCache = {} // 本地商品缓存
export default {
  components: { vkDataInputNumberBox },
  ...
  methods: {
    ...
    init(notAutoClick) {
      ...
      that.shopItemInfo = {}
      const specListName = that.specListName
    ...
    updateGoodsInfo(goodsInfo) {
      ...
      // 更新选中sku的库存信息
      const select_sku_info = that.getListItem(
    ...
    async open() {
      ...
      const skuListName = that.skuListName
      // 先获取缓存中的商品信息
      let useCache = false
    ...
    checkSelectShop() {
      ...
      if (that.selectArr.every((item) => item != '')) {
        ...
        const stock = that.selectShop[that.stockName]
    ...
    checkInpath(clickIndex) {
      ...
      const specList = that.goodsInfo[specListName]
      for (let i = 0, len = specList.length; i < len; i++) {
        if (i == clickIndex) {
          continue
        }
        const len2 = specList[i].list.length
        for (let j = 0; j < len2; j++) {
          if (that.subIndex[i] != -1 && j == that.subIndex[i]) {
            continue
          }
          const choosed_copy = [...that.selectArr]
          that.$set(choosed_copy, i, specList[i].list[j].name)
          const choosed_copy2 = choosed_copy.filter(
    ...
    checkItem() {
      ...
      const { stockName } = that
      const skuListName = that.skuListName
      // 去除库存小于等于0的商品sku
      const originalSkuList = that.goodsInfo[skuListName]
      const skuList = []
      let stockNum = 0
    ...
    checkSelectComplete(obj = {}) {
      ...
      const { selectShop, selectNum, stockText, stockName } = that
    ...
    autoClickSku() {
      ...
      const specListArr = that.goodsInfo[that.specListName]
      if (specListArr.length == 1) {
        const specList = specListArr[0].list
        for (let i = 0; i < specList.length; i++) {
          const sku = that.getListItem(skuList, that.skuArrName, [specList[i].name])
    ...
    themeColorFn(name) {
      const that = this
      const { theme, themeColor } = that
      const color = that[name] ? that[name] : themeColor[theme][name]
      return color
    },
    defaultSelectSku() {
      const that = this
      const { defaultSelect } = that
    ...
    selectSku(obj = {}) {
      const that = this
      const { sku: skuArr, num: selectNum } = obj
      const specListArr = that.goodsInfo[that.specListName]
      if (skuArr && specListArr.length === skuArr.length) {
        ...
        for (let index = 0; index < skuArr.length; index++) {
          const skuName = skuArr[index]
          const specList = specListArr[index].list
          const index1 = index
          const index2 = that.getListIndex(specList, 'name', skuName)
          if (index2 == -1) {
            clickKey = false
            break
          }
    ...
    previewImage() {
      const that = this
      const { selectShop, goodsInfo, goodsThumbName } = that
      const src = selectShop.image ? selectShop.image : goodsInfo[goodsThumbName]
      if (src) {
        uni.previewImage({
          urls: [src],
        })
    ...
    getMaxStock() {
      let maxStock = 0
      const that = this
      const { selectShop = {}, goodsInfo = {}, skuListName, stockName } = that
      if (selectShop[stockName]) {
        maxStock = selectShop[stockName]
      } else {
        const skuList = goodsInfo[skuListName]
  ...
  computed: {
    ...
    maxBuyNumCom() {
      const that = this
      const maxStock = that.getMaxStock()
    ...
    isManyCom() {
      const that = this
      const { goodsInfo, defaultSingleSkuName, specListName } = that
      let isMany = true
      if (
    ...
    priceCom() {
      let str = ''
      const that = this
      const { selectShop = {}, goodsInfo = {}, skuListName, skuIdName } = that
      if (selectShop[skuIdName]) {
        ...
        if (skuList && skuList.length > 0) {
          ...
          const min = that.priceFilter(Math.min(...valueArr))
          const max = that.priceFilter(Math.max(...valueArr))
          if (min === max) {
            str = min + ''
          } else {
            str = `${min} - ${max}`
